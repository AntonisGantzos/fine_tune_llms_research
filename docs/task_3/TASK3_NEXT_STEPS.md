...
| 4. Training notebook | ✅ Built, ❌ not yet run to completion | Model load, QLoRA config, 8 pre-flight checks, train + adapter save all present; **run v11 was killed at Kaggle's 12 h wall** (see run history below) |
| 5. Evaluation cells | ✅ Built, ❌ never reached | Batched greedy generation + valid-label rate / accuracy / macro+micro-F1 / per-label / confusions → `eval_metrics.json` + `eval_report.txt` |
